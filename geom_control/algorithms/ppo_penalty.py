...
import torch.optim
from torch.linalg import det, norm
import tqdm
...
from torchrl.objectives import KLPENPPOLoss, ClipPPOLoss
from torchrl.objectives.value.advantages import GAE
from torchrl.record.loggers import generate_exp_name, get_logger

# from backpack import backpack, extend
# from backpack.extensions import KFAC, backprop_extension
...
class ppo_penalty_gae:
    ...
    def run(self):
        ...
        algorithm_name = "PENALTY_Proximal_Policy_Optimization"
        ...
        # FOR BACKPACK: need to fix
        # actor_optim = KFACOptimizer(actor.parameters(), step_size=lrate, damping=1e-5)
        # critic_optim = KFACOptimizer(critic.parameters(), step_size=lrate, damping=1e-5)
...
#----------------- MAIN LOOP------------------------------------------------------------#
        for i, data in enumerate(collector):
            ...
            # #-----------EMPIRICAL FISHER NORM/TRACE with decay INIT--------
            # # trace init
            # avg_empirical_fisher_trace = torch.tensor(0)
            # # norm init
            # param_size = get_param_vector_length(actor)
            ...
            for j in range(loss_epochs):
                ...
                for k, batch in enumerate(data_buffer):
                    ...
                    # for backpack to work, need to write a custom extension for
                    # torch.nn.distributions.continuous.AddStateIndependentNormalScale
                    #--------------------------------------------------------
